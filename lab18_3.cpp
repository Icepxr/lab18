...
student * findstudent(vector<student> &allstudents, int key) { // ใช้ reference เพื่อลดการ copy
...
int main() {
    ...
    while (getline(course_file, textline)) {
        if (state == 1) {
            ...
            c.id = stoi(textline.substr(loc + 1, 5));
            getline(course_file, textline); // ข้าม \"> Lecturers"
